...
function startApp(argument) {
    ...
    function createCard(item, elementID, qun) {
        ...
        var temp = '<div id="productCard" card_id="card_' + id + '" class=" lighten-5" style="padding: 4px;">' +
            ...
            '<div onclick="minus(' + id + ')" class="col s3 center"><i class="material-icons PlusMinus">navigate_before</i></div>' +
            ...
            '<div onclick="plus(' + id + ')" class="col s3 center"><i class="material-icons PlusMinus">navigate_next</i></div>' +
    ...
    function favLocationsList(location) {

        var temp = '<ul>' +
            '<li id="" class="item-content" onclick="selectFavLocation(' + location.lat + ',' + location.lng + ')">' +
    ...
    sendOrder = function(arg) {
        ...
        myApp.showIndicator();
        $$.post("http://63.142.251.169/jusb/restapi/placeOrder.php", arg, function(data) {
            myApp.hideIndicator();
        });
    ...
    loginCheck = function(arg) {
        ...
        myApp.showIndicator();
        $$.post("http://63.142.251.169/jusb/restapi/login.php", formData, function(data) {
            myApp.hideIndicator();
            console.log(data);
    ...
    checkout = function(argument) {
        ...
        if (mainOrdersArray.length > 0) {
            ...
            saveOrderLocationAndSendOrder(2);
            // showMapModel();
            // myApp.confirm('هل أنت متأكد من عمليت الشراء', 'FEELHUNGRY',
            //     function() {
            //         var userData = storage.getItem('userData');
            //         var orders = storage.getItem('orders');
            //         var ordersAsArray = eval(orders);
            //         myApp.showIndicator();
            //         $.post("http://63.142.251.169/jusb/restapi/placeOrder.php", { ordersAsArray, userData , orderLocation }, function(data) {
            //             myApp.hideIndicator();
            //             console.log();
    ...
    signup = function(argument) {
        ...
        if (formData.email == "" || formData.name == "" || formData.password == "" || formData.phoneNumber == "" || formData.studentNumber == "") {
            ...
            myApp.showIndicator();
            $.post('http://63.142.251.169/jusb/restapi/createCustomer.php', formData, function(data, textStatus, xhr) {
                myApp.hideIndicator();
                console.log(data);
                if (data.startsWith("{")) {
    ...
    myOrder = function(argument) {
        ...
        $$.get("http://63.142.251.169/jusb/restapi/myOrder.php", { userID: userID }, function(data) {
            myApp.hideIndicator();
    ...
    saveOrderLocationAndSendOrder = function(flag) {
        if (flag == 0) {
            ...
            myApp.confirm('هل أنت متأكد من عمليت الشراء', 'FEELHUNGRY',
                function() {
                    ...
                    var ordersAsArray = eval(orders);
                    myApp.showIndicator();
                    $.post("http://63.142.251.169/jusb/restapi/placeOrder.php", { preparedOrdersArray, userData, orderLocation }, function(data) {
                    	console.log('hi');
                    	var dataAsJSON = $.parseJSON(data);

                    	console.log(dataAsJSON);
                        if (dataAsJSON.info) {
                            myApp.alert(dataAsJSON.info);
                        } else {
                            myApp.hideIndicator();
        ...
        if (flag == 1) {
            myApp.prompt('Location Name', 'FEELHUNGRY', function(value) {
    ...
    openProductPageForCat = function(id) {
        ...
        if (reqArrayCate.includes(id)) {
            ...
            $$.getJSON("http://63.142.251.169/jusb/restapi/getSpecificCateProducts.php", { id: id }, function(data) {
                myApp.hideIndicator();
    ...
    createSubCateTabs = function(mainCateID) {
        ...
        for (var i = 0; i < mainProductsArray.length; i++) {
            mainProductsArray[i].categories = _.sortBy(mainProductsArray[i].categories, ['id']);
        }



        mainProductsArray.forEach(function(element1) {
            if (_.find(element1.categories, { 'id': mainCateID })) {
    ...
    selectFavLocation = function(locationLat, locationLng) {
        var obj = { "lat": locationLat, "lng": locationLng };
        var locationInfo = JSON.stringify(obj);
        storage.setItem('orderLocation', locationInfo);
        saveOrderLocationAndSendOrder(2);
    }
    ...
    myApp.onPageInit('products', function(page) {
        ...
        if (getProdect == 0) {
            // myApp.showIndicator();
            // $$.getJSON("http://63.142.251.169/jusb/restapi/products.php", function(data) {
            //     myApp.hideIndicator();
            //     data.forEach(function(post) {
            //         storage.setItem(post.id, JSON.stringify(post));
            //         createCard(post, "productList", 0);
    ...
    myApp.onPageInit('favoriteLocation', function(page) {
        ...
        listfavLocations = function(argument) {
            ...
            for (var i = 0; i < objFavLocations.length; i++) {
                ...
                favLocationsList(objFavLocations[i]);
            }
        }
        listfavLocations();
    });
    ...
    myApp.onPageInit('searchResult', function(page) {
        ...
        myApp.showIndicator();
        $$.getJSON("http://63.142.251.169/jusb/restapi/searchResult.php", { 'searchedProduct': searchedProduct }, function(data) {
            myApp.hideIndicator();
            console.log(data);
            data.forEach(function(post) {
                if (!_.find(mainProductsArray, post)) {
    ...
    setTimeout(function() {
        ...
        if (userDataJSON) {
            // myApp.showIndicator();
            // $.post('http://63.142.251.169/jusb/restapi/retrieveCustomer.php', { 'id': userDataJSON.id }, function(data, textStatus, xhr) {
            //     myApp.hideIndicator();
    ...
    retrieveCategories = function(argument) {
        myApp.showIndicator();
        $$.getJSON("http://63.142.251.169/jusb/restapi/retrieveCategories.php", function(data) {
            myApp.hideIndicator();
            console.log(data);
